    ...
    I2C: embedded_hal::i2c::I2c<Error = E>,
{
    ...
    pub fn init(&self, i2c: &mut I2C) -> Result<(), Error> {
    ...
    pub fn get_touch(&self, i2c: &mut I2C) -> Result<Option<Point>, Error> {
    ...
    pub fn get_multi_touch(
        &self,
        i2c: &mut I2C,
    ) -> Result<heapless::Vec<Point, MAX_NUM_TOUCHPOINTS>, Error> {
        let num_touch_points = self.get_num_touch_points(i2c)?;
    ...
    fn get_num_touch_points(&self, i2c: &mut I2C) -> Result<usize, Error> {
        // read coords
        let mut read = [0u8; 1];
    ...
    fn write(&self, i2c: &mut I2C, register: u16, value: u8) -> Result<(), Error> {
        let register = register.to_be_bytes();
        let cmd = [register[0], register[1], value];
        i2c.write(self.i2c_addr, &cmd).map_err(|_e| Error::I2C)
    }

    fn read(&self, i2c: &mut I2C, register: u16, buf: &mut [u8]) -> Result<(), Error> {
        i2c.write_read(self.i2c_addr, &register.to_be_bytes(), buf)
            .map_err(|_e| Error::I2C)
    }
}
    ...
    I2C: embedded_hal_async::i2c::I2c<Error = E>,
{
    ...
    /// Checks the ProductId for a "911\0" string response and resets the status register
    /// Only needs to be called once on startup
    pub async fn init(&self, i2c: &mut I2C) -> Result<(), Error> {
        // switch to command mode
    ...
    /// Gets a single touch point
    /// Returns Ok(None) for release, Some(point) for press or move and Err(Error::NotReady) for no data
    pub async fn get_touch(&self, i2c: &mut I2C) -> Result<Option<Point>, Error> {
    ...
    async fn get_num_touch_points(&self, i2c: &mut I2C) -> Result<usize, Error> {
        // read coords
        let mut read = [0u8; 1];
        self.read(i2c, GT911_TOUCHPOINT_STATUS_REG, &mut read)
            .await?;
    ...
    async fn write(&self, i2c: &mut I2C, register: u16, value: u8) -> Result<(), Error> {
        let register = register.to_be_bytes();
        let cmd = [register[0], register[1], value];
        i2c.write(self.i2c_addr, &cmd)
            .await
            .map_err(|_e| Error::I2C)
    }

    async fn read(&self, i2c: &mut I2C, register: u16, buf: &mut [u8]) -> Result<(), Error> {
        i2c.write_read(self.i2c_addr, &register.to_be_bytes(), buf)
            .await
            .map_err(|_e| Error::I2C)
    }
